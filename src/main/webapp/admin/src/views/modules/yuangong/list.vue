<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yuangong','培训需求') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="peixunxuqiuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'培训需求':'' }}</el-button>
                <el-button v-if="isAuth('yuangong','培训需求') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="peixunxuqiuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'培训需求':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuangong','培训需求') && contents.tableBtnIcon == 0" type="success" size="mini" @click="peixunxuqiuCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'培训需求':'' }}</el-button>
    ...
    <peixunxuqiu-cross-add-or-update v-if="peixunxuqiuCrossAddOrUpdateFlag" :parent="this" ref="peixunxuqiuCrossaddOrUpdate"></peixunxuqiu-cross-add-or-update>
...
import peixunxuqiuCrossAddOrUpdate from "../peixunxuqiu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      peixunxuqiuCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    peixunxuqiuCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.peixunxuqiuCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.peixunxuqiuCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.peixunxuqiuCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
